...
if ($DryRun) { Write-Host "*** DRY RUN - NO VIDEO FILES WILL BE COPIED ***" }
Write-Host ""
...
for ($i=0; $i -lt $files.Count; $i += 50) {
    ...
    foreach ($record in $records) {
        ...
        if ($needsCopy) {
            Write-Host "$(if($destExists){'UPDATE'}else{'NEW   '})  videos\$relative"
            if (!$DryRun) {
...
Write-Host "Video files copied/updated:      $copied"
