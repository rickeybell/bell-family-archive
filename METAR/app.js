...
import {alwaysVisibleTrafficIdentifiers,klkrAreaModeEnabled,klkrCircleEnabled,klkrTrafficArea,radarCoverageWithinKlkrArea,trafficButtonDetail,trafficDataFresh,trafficHasConstantLabel,trafficPollingNeeded,trafficVisibleAtZoom,trafficWithinKlkrArea} from './traffic-display.mjs?v=20260921-klkr-area4';
...
function klkrVisibleOnMap(){const [x,y]=xy(klkrTrafficArea.lon,klkrTrafficArea.lat);return x>=0&&x<=width&&y>=0&&y<=height;}
function klkrAreaActive(){return klkrAreaModeEnabled({trafficOn,zoom,klkrVisible:klkrVisibleOnMap()});}
function klkrCircleActive(){return klkrCircleEnabled({trafficOn,zoom,klkrVisible:klkrVisibleOnMap(),radarCoverage:klkrRadarCoverage});}
function updateTrafficModeUi(areaMode=klkrAreaActive()){const button=$('traffic-toggle'),mode=areaMode?'area':'regional';button.classList.toggle('klkr-area',areaMode);if(button.dataset.mode!==mode){button.dataset.mode=mode;const label=document.createElement('span'),detail=document.createElement('small');label.className='traffic-label';label.textContent='GA Traffic';detail.textContent=trafficButtonDetail(areaMode);button.replaceChildren(label,document.createElement('br'),detail);}button.title=areaMode?'Show GA traffic below 180 knots within 50 statute miles of KLKR; the area circle appears below 50% radar coverage':'Show GA traffic below 180 knots at 150% and closer; listed aircraft appear at every zoom';}
function drawKlkrTrafficArea(){const [x,y]=xy(klkrTrafficArea.lon,klkrTrafficArea.lat),east=xy(klkrTrafficArea.lon+klkrTrafficArea.radiusSm/(69.172*Math.cos(klkrTrafficArea.lat*Math.PI/180)),klkrTrafficArea.lat),radius=Math.abs(east[0]-x),label='GA Traffic',areaColor='#be9df680';trafficContext.save();trafficContext.fillStyle='#be9df60a';trafficContext.strokeStyle=areaColor;trafficContext.lineWidth=2.5;trafficContext.setLineDash([9,6]);trafficContext.beginPath();trafficContext.arc(x,y,radius,0,Math.PI*2);trafficContext.fill();trafficContext.stroke();trafficContext.setLineDash([]);trafficContext.font='bold 12px Inter, Segoe UI, sans-serif';const labelWidth=trafficContext.measureText(label).width+12,labelX=x+radius*.72-labelWidth/2,labelY=y+radius*.13;trafficContext.fillStyle='#7c3aed';trafficContext.fillRect(labelX,labelY-14,labelWidth,20);trafficContext.fillStyle='#fff';trafficContext.fillText(label,labelX+6,labelY);trafficContext.restore();}
function visibleTraffic(){if(!trafficOn)return [];const bounds=visibleBounds(30),areaMode=klkrAreaActive();return trafficAircraft.filter(aircraft=>(areaMode?trafficWithinKlkrArea(aircraft):trafficVisibleAtZoom(aircraft,zoom,alwaysVisibleTrafficIdentifiers))&&aircraft.lon>=bounds.west&&aircraft.lon<=bounds.east&&aircraft.lat>=bounds.south&&aircraft.lat<=bounds.north).slice(0,75);}
...
function drawTraffic(){
 trafficContext.clearRect(0,0,width,height);trafficScreen=[];const areaMode=klkrAreaActive();updateTrafficModeUi(areaMode);if(!trafficOn){$('traffic-tooltip').hidden=true;return;}if(klkrCircleActive())drawKlkrTrafficArea();const aircraftList=visibleTraffic();trafficContext.lineJoin='round';
...
function trafficQuery(){if(klkrAreaActive()){const latRadius=klkrTrafficArea.radiusSm/69.172,lonRadius=latRadius/Math.cos(klkrTrafficArea.lat*Math.PI/180);return {south:klkrTrafficArea.lat-latRadius,north:klkrTrafficArea.lat+latRadius,west:klkrTrafficArea.lon-lonRadius,east:klkrTrafficArea.lon+lonRadius};}const bounds=visibleBounds(20);return {south:Math.max(-90,bounds.south),north:Math.min(90,bounds.north),west:Math.max(-180,bounds.west),east:Math.min(180,bounds.east)};}
...
async function analyzeKlkrRadar(source){if(!source||source===klkrRadarSource)return;klkrRadarSource=source;let coverage=null;try{const response=await fetch(source,{cache:'force-cache',signal:AbortSignal.timeout(25000)});if(!response.ok)throw Error('Radar image unavailable');const bitmap=await createImageBitmap(await response.blob()),canvas=document.createElement('canvas');canvas.width=380;canvas.height=450;const context=canvas.getContext('2d',{willReadFrequently:true});context.drawImage(bitmap,0,0,canvas.width,canvas.height);bitmap.close?.();coverage=radarCoverageWithinKlkrArea(context.getImageData(0,0,canvas.width,canvas.height).data,canvas.width,canvas.height,radarBounds);}catch(error){console.error(error);}if(source!==klkrRadarSource)return;klkrRadarCoverage=coverage;drawTraffic();}
...
$('radar-image').addEventListener('load',event=>{event.currentTarget.classList.remove('unavailable');$('radar-toggle').classList.remove('radar-error');$('radar-toggle').title='Show or hide NOAA weather radar';analyzeKlkrRadar(event.currentTarget.getAttribute('href'));});
$('radar-image').addEventListener('error',()=>{klkrRadarCoverage=null;drawTraffic();$('radar-image').classList.add('unavailable');$('radar-toggle').classList.add('radar-error');$('radar-toggle').title='Radar is temporarily unavailable';});
