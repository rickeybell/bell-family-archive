...
function renderDetail(){
 ...
 $('stale-note').hidden=status!=='STALE';$('wind').textContent=wind(r);$('visibility').textContent=r?.visib!=null?`${r.visib} SM`:'Unavailable';$('ceiling').textContent=ceiling(r);$('temperature').textContent=Number.isFinite(r?.temp)?`${Math.round(r.temp*9/5+32)}°F / ${r.temp}°C`:'Unavailable';const hasFuel=Number.isFinite(s.fuel100LL),fuelDetail=$('fuel-detail');fuelDetail.hidden=!hasFuel;if(hasFuel){$('fuel-price-detail').textContent=`$${s.fuel100LL.toFixed(2)} / gal`;const service=s.fuelService?`${s.fuelService.replace('-', ' ')} · `:'';$('fuel-detail-note').textContent=`${service}reported ${s.fuelPriceReported||'date unavailable'}`;}$('raw').textContent=fuelOnly?`${s.id} does not publish METAR observations.`:r?.rawOb||'No METAR is available for this airport.';
}
