...
function draw(){
 ...
 for(const s of shown){
  const n=nodes.get(s.id),[x,y]=positions.get(s.id);n.g.setAttribute('transform',`translate(${x},${y})`);
   ...
   const fixedChoice=s.id==='KCGC'&&Math.abs(zoom-2)<.15?[-labelWidth/2,30]:s.id==='KLKR'&&zoom>=1.5?[-38-labelWidth,-22]:zoom<1.01?(s.id==='KCUB'?[18,-20]:s.id==='KCAE'?[-15-labelWidth,5]:null):null,choices=fixedChoice?[fixedChoice]:[[15,5],[-15-labelWidth,5],[-labelWidth/2,-16],[-labelWidth/2,28],[18,-16],[-18-labelWidth,-16],[18,28],[-18-labelWidth,28],[24,46],[-24-labelWidth,46],[20,-34],[-20-labelWidth,-34]];
