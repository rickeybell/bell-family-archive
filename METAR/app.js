...
import {alwaysVisibleTrafficIdentifiers,klkrAreaModeEnabled,klkrTrafficArea,radarCoverageWithinKlkrArea,trafficHasConstantLabel,trafficPollingNeeded,trafficVisibleAtZoom,trafficWithinKlkrArea} from './traffic-display.mjs?v=20260921-klkr-area1';
...
function klkrAreaActive(){return klkrAreaModeEnabled({trafficOn,zoom,klkrVisible:klkrVisibleOnMap(),radarCoverage:klkrRadarCoverage});}
function updateTrafficModeUi(areaMode=klkrAreaActive()){const button=$('traffic-toggle'),mode=areaMode?'area':'regional';button.classList.toggle('klkr-area',areaMode);if(button.dataset.mode!==mode){button.dataset.mode=mode;button.replaceChildren('GA Traffic');if(areaMode){button.append(document.createElement('br'));const detail=document.createElement('small');detail.textContent='KLKR Area';button.append(detail);}}button.title=areaMode?'GA traffic within 30 statute miles of KLKR; shown while radar coverage in the area is below 20%':'Show GA traffic at 150% and closer; listed aircraft appear at every zoom';}
function drawKlkrTrafficArea(){const [x,y]=xy(klkrTrafficArea.lon,klkrTrafficArea.lat),east=xy(klkrTrafficArea.lon+klkrTrafficArea.radiusSm/(69.172*Math.cos(klkrTrafficArea.lat*Math.PI/180)),klkrTrafficArea.lat),radius=Math.abs(east[0]-x),label='GA Traffic';trafficContext.save();trafficContext.fillStyle='#7c3aed14';trafficContext.strokeStyle='#7c3aed';trafficContext.lineWidth=2.5;trafficContext.setLineDash([9,6]);trafficContext.beginPath();trafficContext.arc(x,y,radius,0,Math.PI*2);trafficContext.fill();trafficContext.stroke();trafficContext.setLineDash([]);trafficContext.font='bold 12px Inter, Segoe UI, sans-serif';const labelWidth=trafficContext.measureText(label).width+12,labelX=x+radius*.55-labelWidth/2,labelY=y-radius*.82;trafficContext.fillStyle='#07151fee';trafficContext.fillRect(labelX,labelY-14,labelWidth,20);trafficContext.fillStyle='#c4b5fd';trafficContext.fillText(label,labelX+6,labelY);trafficContext.restore();}
...
function drawTraffic(){
 trafficContext.clearRect(0,0,width,height);trafficScreen=[];const areaMode=klkrAreaActive();updateTrafficModeUi(areaMode);if(!trafficOn){$('traffic-tooltip').hidden=true;return;}if(areaMode)drawKlkrTrafficArea();const aircraftList=visibleTraffic();trafficContext.lineJoin='round';
