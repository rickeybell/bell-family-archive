...
import {alwaysVisibleTrafficIdentifiers,radarCoverageWithinArea,trafficAreaModeEnabled,trafficAreas,trafficButtonDetail,trafficCircleEnabled,trafficDataFresh,trafficHasConstantLabel,trafficPollingNeeded,trafficVisibleAtZoom,trafficWithinArea} from './traffic-display.mjs?v=20260921-regional-areas1';
...
function activeTrafficAreas(){const areas=visibleTrafficAreas();return trafficAreaModeEnabled({trafficOn,zoom,areaVisible:areas.length>0})?areas:[];}
...
function drawTraffic(){
 trafficContext.clearRect(0,0,width,height);trafficScreen=[];const areas=activeTrafficAreas();updateTrafficModeUi(areas);if(!trafficOn){$('traffic-tooltip').hidden=true;return;}for(const area of visibleTrafficAreas())if(trafficCircleEnabled({trafficOn,zoom,areaVisible:true,radarCoverage:trafficRadarCoverage.get(area.id),maxRadarCoverage:area.maxRadarCoverage}))drawTrafficArea(area);const aircraftList=visibleTraffic();trafficContext.lineJoin='round';
