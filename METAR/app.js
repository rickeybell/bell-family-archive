...
import {alwaysVisibleTrafficIdentifiers,klkrAreaModeEnabled,klkrCircleEnabled,klkrTrafficArea,radarCoverageWithinKlkrArea,trafficDataFresh,trafficHasConstantLabel,trafficPollingNeeded,trafficVisibleAtZoom,trafficWithinKlkrArea} from './traffic-display.mjs?v=20260921-klkr-area3';
...
function updateTrafficModeUi(areaMode=klkrAreaActive()){const button=$('traffic-toggle'),mode=areaMode?'area':'regional';button.classList.toggle('klkr-area',areaMode);if(button.dataset.mode!==mode){button.dataset.mode=mode;const label=document.createElement('span');label.className='traffic-label';label.textContent='GA Traffic';button.replaceChildren(label);if(areaMode){button.append(document.createElement('br'));const detail=document.createElement('small');detail.textContent='KLKR Area';button.append(detail);}}button.title=areaMode?'Show GA traffic within 50 statute miles of KLKR; the area circle appears below 50% radar coverage':'Show GA traffic at 150% and closer; listed aircraft appear at every zoom';}
