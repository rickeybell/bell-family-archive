...
async function initialize(){try{
 const results=await Promise.all(['stations.json?v=20260914-pyg125','states.json?v=regional1','airspaces.json?v=restricted1'].map(async url=>{const r=await fetch(url);if(!r.ok)throw Error('Map asset unavailable');return r.json();}));
