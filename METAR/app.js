...
function visibleTraffic(){if(!trafficOn)return [];const bounds=visibleBounds(30);return trafficAircraft.filter(aircraft=>trafficVisibleAtZoom(aircraft,zoom,alwaysVisibleTrafficIdentifiers)&&aircraft.lon>=bounds.west&&aircraft.lon<=bounds.east&&aircraft.lat>=bounds.south&&aircraft.lat<=bounds.north).slice(0,75);}
...
function trafficQuery(){const bounds=visibleBounds(20);return {south:Math.max(-90,bounds.south),north:Math.min(90,bounds.north),west:Math.max(-180,bounds.west),east:Math.min(180,bounds.east)};}
async function loadTraffic(){
 ...
 try{const query=trafficQuery(),url=new URL(`${serviceBase}/traffic`);for(const [name,value] of Object.entries(query))url.searchParams.set(name,value.toFixed(3));const response=await fetch(url,{cache:'no-store',signal:AbortSignal.timeout(9000)}),data=await response.json();if(!response.ok||!Array.isArray(data.aircraft))throw Error('Traffic unavailable');trafficAircraft=data.aircraft;trafficFetchedAt=Date.parse(data.fetchedAt)||Date.now();const button=$('traffic-toggle');button.classList.remove('traffic-error');button.title='Show GA traffic at 150% and closer; listed aircraft appear at every zoom';drawTraffic();}
