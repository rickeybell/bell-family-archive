...
function visibleTraffic(){if(!trafficOn||zoom<1)return [];const bounds=visibleBounds(30);return trafficAircraft.filter(aircraft=>aircraft.lon>=bounds.west&&aircraft.lon<=bounds.east&&aircraft.lat>=bounds.south&&aircraft.lat<=bounds.north).slice(0,75);}
function positionTrafficTooltip(aircraft,event){const box=$('traffic-tooltip');if(!aircraft){box.hidden=true;return;}box.replaceChildren();const id=document.createElement('b');id.textContent=aircraft.id;box.append(id,document.createTextNode(`\n${Math.round(aircraft.speed)} kt\n${aircraft.altitude.toLocaleString()} ft MSL\n${trafficAge(aircraft)} sec old`));box.hidden=false;const point=xy(aircraft.lon,aircraft.lat),rect=map.getBoundingClientRect(),anchorX=event?.clientX??rect.left+point[0]*rect.width/width,anchorY=event?.clientY??rect.top+point[1]*rect.height/height,tip=box.getBoundingClientRect(),gap=14;box.style.left=`${Math.max(8,Math.min(innerWidth-tip.width-8,anchorX+gap))}px`;box.style.top=`${Math.max(8,Math.min(innerHeight-tip.height-8,anchorY+gap))}px`;}
function drawTraffic(){
 trafficContext.clearRect(0,0,width,height);trafficScreen=[];if(!trafficOn||zoom<1){$('traffic-tooltip').hidden=true;return;}const aircraftList=visibleTraffic(),labels=zoom>=2.5,boxes=[];trafficContext.lineJoin='round';trafficContext.font='10px Consolas, monospace';
 for(const aircraft of aircraftList){const [x,y]=xy(aircraft.lon,aircraft.lat),angle=(Number(aircraft.track)||0)*Math.PI/180,selectedAircraft=aircraft.id===trafficSelected;trafficScreen.push({...aircraft,x,y});trafficContext.save();trafficContext.translate(x,y);trafficContext.rotate(angle);trafficContext.beginPath();trafficContext.moveTo(0,-8);trafficContext.lineTo(5,7);trafficContext.lineTo(0,4);trafficContext.lineTo(-5,7);trafficContext.closePath();trafficContext.fillStyle=selectedAircraft?'#fff':'#76d9ff';trafficContext.strokeStyle='#06131b';trafficContext.lineWidth=2;trafficContext.stroke();trafficContext.fill();trafficContext.restore();
  if(labels){const lines=[aircraft.id,`${Math.round(aircraft.speed)} kt`,`${aircraft.altitude.toLocaleString()} ft MSL`,`${trafficAge(aircraft)} sec`],labelWidth=Math.max(...lines.map(line=>trafficContext.measureText(line).width),54)+10,choices=[[11,-22],[-labelWidth-11,-22],[11,10],[-labelWidth-11,10]],heightBox=48;let choice=choices.find(([dx,dy])=>!boxes.some(box=>x+dx<box.right&&x+dx+labelWidth>box.left&&y+dy<box.bottom&&y+dy+heightBox>box.top))||choices[0],left=x+choice[0],top=y+choice[1];boxes.push({left,right:left+labelWidth,top,bottom:top+heightBox});trafficContext.fillStyle='#07151fe6';trafficContext.strokeStyle='#76d9ff99';trafficContext.lineWidth=1;trafficContext.fillRect(left,top,labelWidth,heightBox);trafficContext.strokeRect(left+.5,top+.5,labelWidth-1,heightBox-1);trafficContext.font='10px Consolas, monospace';trafficContext.fillStyle='#dff8ff';lines.forEach((line,index)=>trafficContext.fillText(line,left+5,top+11+index*11));}
 }
...
async function loadTraffic(){
 ...
 finally{trafficLoading=false;if(trafficOn&&zoom>=1&&!document.hidden)trafficTimer=setTimeout(loadTraffic,10000);}
}
...
function drawTfrs(){
 ...
 for(const feature of tfrs){const properties=feature.properties||{},expires=Date.parse(properties.expires);if(Number.isFinite(expires)&&expires<=now||!intersectsBounds(feature,bounds))continue;const d=geometryPath(feature.geometry);if(!d)continue;const starts=Date.parse(properties.effective),upcoming=Number.isFinite(starts)&&starts>now,active=Number.isFinite(starts)&&starts<=now;el('path',{d,class:`tfr${upcoming?' upcoming':''}${active?' active':''}`,'fill-rule':'evenodd'},layer);}
}
...
function draw(){
 ...
 syncAloftWorkerView();drawTraffic();if(trafficOn&&zoom>=1&&!document.hidden&&!trafficLoading&&!trafficTimer)loadTraffic();
}
...
function renderDetail(){
 ...
 $('stale-note').hidden=status!=='STALE';$('stale-note').textContent=usableReport?'Older observation. Current conditions may differ.':'Observation is over two hours old. Weather details are hidden.';$('wind').textContent=wind(usableReport);$('visibility').textContent=usableReport?.visib!=null?`${usableReport.visib} SM`:'Unavailable';$('ceiling').textContent=ceiling(usableReport);$('temperature').textContent=Number.isFinite(usableReport?.temp)?`${Math.round(usableReport.temp*9/5+32)}°F / ${usableReport.temp}°C`:'Unavailable';const hasFuel=Number.isFinite(s.fuel100LL),fuelDetail=$('fuel-detail');fuelDetail.hidden=!hasFuel;if(hasFuel){$('fuel-price-detail').textContent=`$${s.fuel100LL.toFixed(2)} / gal`;const service=s.fuelService?`${s.fuelService.replace('-', ' ')} · `:'';$('fuel-detail-note').textContent=`${service}reported ${s.fuelPriceReported||'date unavailable'}`;}$('raw').textContent=noMetar?`${s.id} does not publish METAR observations.`:usableReport?.rawOb||(!observationUsable(r)&&r?'Observation is over two hours old and is hidden.':'No METAR is available for this airport.');
}
