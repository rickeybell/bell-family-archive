...
function Process-Batch {
    ...
    $args = @("-json", "-Subject", "-Keywords", "-HierarchicalSubject")
    ...
    if ($LASTEXITCODE -ne 0 -and !$json) {
        foreach ($file in $Files) {
            $script:stats.Errors++
            Write-Warning "ExifTool could not read: $($file.FullName)"
        }
        return
    }
    ...
    foreach ($record in $records) {
        $bySource[[string]$record.SourceFile] = $record
    }

    foreach ($file in $Files) {
        $script:stats.MetadataRead++
        $record = $bySource[$file.FullName]

        if (!$record) {
            $script:stats.Errors++
            Write-Warning "No metadata returned for: $($file.FullName)"
            continue
        }
...
if ($DryRun) {
    Write-Host "Dry run complete. Nothing was copied or deleted."
} else {
