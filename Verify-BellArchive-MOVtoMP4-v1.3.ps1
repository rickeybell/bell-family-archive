...
function GPSNumbers($Values) {
    ...
    foreach ($value in @($Values)) {
        $text = "$value"
        foreach ($match in [regex]::Matches($text, '(?i)(\d+)\s*deg\s*(\d+)''\s*([0-9.]+)""\s*([NSEW])')) {
            $decimal = [double]$match.Groups[1].Value +
                       ([double]$match.Groups[2].Value / 60) +
                       ([double]$match.Groups[3].Value / 3600)
            $direction = $match.Groups[4].Value.ToUpperInvariant()
            if ($direction -in @('S','W')) { $decimal = -$decimal }
            if ($direction -in @('N','S')) { $lat += $decimal } else { $lon += $decimal }
        }
        foreach ($match in [regex]::Matches($text, '(?i)([-+]?[0-9]+(?:\.[0-9]+)?)\s*m(?:\s|$)')) {
            $alt += [double]$match.Groups[1].Value
        }
    }

    return [pscustomobject]@{
        Latitude  = @($lat | Sort-Object -Unique)
        Longitude = @($lon | Sort-Object -Unique)
        Altitude  = @($alt | Sort-Object -Unique)
    }
}

function NumericSetsMatch($A,$B,[double]$Tolerance) {
    $A = @($A)
    $B = @($B)
    if ($A.Count -eq 0 -or $B.Count -eq 0) { return $A.Count -eq $B.Count }
    foreach ($x in $A) {
        if (@($B | Where-Object { [math]::Abs([double]$_ - [double]$x) -le $Tolerance }).Count -eq 0) { return $false }
    }
    foreach ($x in $B) {
        if (@($A | Where-Object { [math]::Abs([double]$_ - [double]$x) -le $Tolerance }).Count -eq 0) { return $false }
    }
    return $true
}
...
function CmpGps($A,$B) {
    ...
    $an = GPSNumbers $A
    $bn = GPSNumbers $B
    $coordinatesMatch = (NumericSetsMatch $an.Latitude $bn.Latitude 0.00002) -and
                        (NumericSetsMatch $an.Longitude $bn.Longitude 0.00002)
    $altitudesMatch = NumericSetsMatch $an.Altitude $bn.Altitude 0.02

    return [pscustomobject]@{
        Status = if ($coordinatesMatch -and $altitudesMatch) { "OK" } else { "DIFF" }
        A = ($A -join " | ")
        B = ($B -join " | ")
    }
}
...
Write-Host ""
Write-Host "======================================================================"
Write-Host "Bell Family Archive - MOV to MP4 Metadata Verifier v1.3"
Write-Host "======================================================================"
...
foreach ($mov in $movs) {
    ...
    try {
        ...
        # QuickTime containers may expose the same GPS values through multiple tags
        # with slightly different formatting. Compare parsed coordinates (about 2 m
        # tolerance) and altitude (2 cm tolerance), while retaining raw values in CSV.
        $gps = CmpGps (Values $a $gpsNames) (Values $b $gpsNames)
        $dates = Cmp (Values $a $dateNames) (Values $b $dateNames)
        $side = Sidecar $mov $mp4

        $critical = @(
            @($tags.Status,$people.Status,$captions.Status,$ratings.Status,$gps.Status,$side.Status) |
