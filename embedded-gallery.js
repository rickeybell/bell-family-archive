document.addEventListener("DOMContentLoaded", async () => {
  ...
  const galleryName = host.dataset.galleryName || "This";
  ...
  try {
    ...
    style.dataset.embeddedGallery = "true";
    ...
    runner.dataset.embeddedGallery = "true";
    runner.textContent = galleryScript.textContent;
    document.body.appendChild(runner);
  } catch (error) {
    console.error(`${galleryName} gallery could not be loaded:`, error);
    host.setAttribute("aria-busy", "false");
    host.innerHTML = `<p class="embedded-archive-error">${galleryName}&rsquo;s gallery could not be loaded. Please refresh the page and try again.</p>`;
  }
});
