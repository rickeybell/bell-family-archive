...
function Get-SpecificLancasterTags {
    param([string]$MediaPath)
    ...
    foreach ($tag in $allTags) {
        $t=[string]$tag
        $matches=[regex]::Matches($t,'(?i)Places[/|]South Carolina[/|]Lancaster County[/|][^,]+')
...
foreach ($Item in $Items) {
    ...
    if (-not $hasGps) {
        $Rows += [PSCustomObject]@{
            ...
            HasSpecificLancasterTag='';SpecificLancasterTags='';AlreadyLancasterCounty=''
    ...
    if ($inside) {
        $specificTags=@(Get-SpecificLancasterTags -MediaPath $fullPath)
        $hasSpecific=($specificTags.Count -gt 0)
    ...
    $Rows += [PSCustomObject]@{
        ...
        SpecificLancasterTags=($specificTags -join ' | ')
