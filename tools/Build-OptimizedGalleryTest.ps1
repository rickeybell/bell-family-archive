...
foreach ($thumb in $thumbs) {
    ...
    $displayTitle=if ($title){$title}else{$name};$metaLine=(@($date,$location,$people)|Where-Object{![string]::IsNullOrWhiteSpace($_)})-join ' · '
    $cards.Add(@"
...
<script>
const items=[$($items -join ',')];let current=0;const viewer=document.getElementById('viewer'),img=document.getElementById('modalImage'),nameEl=document.getElementById('modalName'),metaEl=document.getElementById('modalMeta'),commentsEl=document.getElementById('modalComments');
function show(i){if(!items.length)return;current=(i+items.length)%items.length;const x=items[current];img.src=x.view;img.alt=x.title||x.name;nameEl.textContent=x.title||x.name;metaEl.textContent=[x.date,x.location,x.people].filter(Boolean).join(' · ');commentsEl.textContent=x.comments||'';viewer.classList.add('open');document.body.style.overflow='hidden'}
...
async function forceDownload(url,filename){try{if(location.protocol==='http:'||location.protocol==='https:'){const r=await fetch(url,{cache:'no-store'});if(!r.ok)throw new Error();const b=await r.blob(),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download=filename||'photo';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),2000);return}}catch(e){}const a=document.createElement('a');a.href=url;a.download=filename||'photo';document.body.appendChild(a);a.click();a.remove()}
