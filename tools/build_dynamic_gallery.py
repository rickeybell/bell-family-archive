...
def cards(items):
    ...
    for x in items:
        ...
        out.append(f'''<article class="archive-card"><a href="{esc(x['path'])}" target="_blank"><img src="{esc(x['path'])}" alt="{esc(title)}" loading="lazy"></a><div class="archive-copy"><h3>{esc(title)}</h3><div class="archive-meta">{esc(meta)}</div>{f'<p>{esc(desc)}</p>' if desc else ''}<div class="archive-file">{esc(x.get('file'))}</div></div></article>''')
    return '\n'.join(out)
