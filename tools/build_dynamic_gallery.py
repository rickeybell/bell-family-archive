...
PHOTO_GROUPS=[
    (
        '784226620_1000948432972754_8357374407785207827_n.jpg',
        '784254923_1001001006300830_4769511192738293737_n.jpg',
    ),
]
...
def group_related_photos(items):
    ordered=sorted(items,key=date_sort_key)
    by_name={str(x.get('file') or pathlib.PurePosixPath(str(x.get('path') or '')).name):x for x in ordered}
    grouped=[];used=set()
    for x in ordered:
        marker=id(x)
        if marker in used:continue
        name=str(x.get('file') or pathlib.PurePosixPath(str(x.get('path') or '')).name)
        names=next((group for group in PHOTO_GROUPS if name in group),None)
        members=[by_name[n] for n in names if n in by_name] if names else []
        if len(members)>1 and all(m.get('media_type') in {None,'photo'} for m in members):
            primary=dict(members[0]);primary['_group_items']=members
            grouped.append(primary);used.update(id(m) for m in members)
        else:
            grouped.append(x);used.add(marker)
    return grouped

def page(title,subtitle,items,filename,empty_html='',after_html=''):
    ordered=group_related_photos(items);years={}
    for x in ordered:
        d=str(x.get('date') or '')
    ...
    for y in sorted(years,key=lambda z:(z=='Unknown',z)):
        cards=[]
        for x in years[y]:
            grouped_photos=x.get('_group_items') or []
            if grouped_photos:
                ...
                media=f'<div class="archive-multi-pic">{"".join(media_parts)}</div>'
                th=f'<h3>{esc(ttl)}</h3>' if ttl else ''
                actions=f'<div class="archive-actions"><button data-view-i="{idx}">View photos</button>{"".join(download_parts)}</div>'
                cards.append(f'''<article class="archive-card archive-card-wide archive-card-group" data-i="{idx}">{media}<div class="archive-copy">{th}<div class="archive-meta">{''.join(lines)}</div>{f'<p>{esc(desc)}</p>' if desc else ''}<div class="archive-file">{len(grouped_photos)} photos</div>{actions}</div></article>''')
                idx+=len(grouped_photos)
                continue
            ...
            th=f'<h3>{esc(ttl)}</h3>' if ttl else ''
            if is_youtube:
                video_label='360° video — Play, then drag' if is_360 else 'YouTube video'
                youtube_action='Open interactive 360° video on YouTube' if is_360 else 'Watch on YouTube'
                iframe_class=' class="archive-video-360"' if is_360 else ''
                youtube_id=(re.search(r'/embed/([^?&#/]+)',v) or [None,''])[1]
                if is_360:
                    thumb=f'https://i.ytimg.com/vi/{youtube_id}/hqdefault.jpg'
                    media=f'<div class="archive-video-wrap archive-video-360-wrap"><img class="archive-video-360-thumb" loading="lazy" src="{esc(thumb)}" alt=""><a class="archive-video-360-launch" href="{esc(o)}" target="_blank" rel="noopener"><strong>Open interactive 360° video on YouTube</strong><span>Drag with the mouse or use W/A/S/D. On a phone or tablet, use the YouTube app.</span></a></div>'
                else:
                    media=f'<div class="archive-video-wrap"><iframe{iframe_class} loading="lazy" src="{esc(v)}" title="{esc(ttl or "YouTube video")}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; xr-spatial-tracking" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe><div class="archive-video-label">{video_label}</div></div>'
                actions=f'<div class="archive-actions"><a href="{esc(o)}" target="_blank" rel="noopener">{youtube_action}</a></div>'
            elif is_video:
            ...
            card_class='archive-card archive-card-360' if is_360 else ('archive-card archive-card-wide' if len(desc) >= 240 else 'archive-card')
    ...
    results=empty_html if empty_html else f'<div class="archive-shell">{sidebar}<div class="archive-results">{"".join(sections)}{after_html}</div></div>'
    group_style='''.archive-card-group{grid-template-rows:auto auto;row-gap:2px;background:#ddd}.archive-card-group>.archive-multi-pic{display:contents}.archive-card-group>.archive-multi-pic>.archive-pic{height:auto;min-height:0}.archive-card-group>.archive-multi-pic>.archive-pic img{width:100%;min-height:0;object-fit:contain}.archive-card-group>.archive-multi-pic>.archive-pic:nth-child(2){grid-column:1;grid-row:1;height:350px}.archive-card-group>.archive-multi-pic>.archive-pic:nth-child(2) img{height:350px}.archive-card-group>.archive-copy{grid-column:2;grid-row:1;background:#fff}.archive-card-group>.archive-multi-pic>.archive-pic:first-child{grid-column:1/-1;grid-row:2;height:280px}.archive-card-group>.archive-multi-pic>.archive-pic:first-child img{height:280px}.archive-card-group .archive-actions{flex-wrap:wrap}@media(max-width:780px){.archive-card-wide.archive-card-group{display:grid;grid-template-columns:1fr;grid-template-rows:auto auto auto}.archive-card-group>.archive-multi-pic>.archive-pic:nth-child(2){grid-column:1;grid-row:1;height:240px}.archive-card-group>.archive-multi-pic>.archive-pic:nth-child(2) img{height:240px}.archive-card-group>.archive-copy{grid-column:1;grid-row:2}.archive-card-group>.archive-multi-pic>.archive-pic:first-child{grid-column:1;grid-row:3;height:240px}.archive-card-group>.archive-multi-pic>.archive-pic:first-child img{height:240px}}''' if any(x.get('_group_items') for x in ordered) else ''
...
@media(max-width:650px){{.archive-card-360{{grid-column:span 1}}}}
...
let current=0,zoomLevel=1,fitScale=1,dragging=false,dragX=0,dragY=0,startLeft=0,startTop=0,pinchStartDistance=0,pinchStartZoom=1;const activePointers=new Map(),V=document.getElementById('viewer'),S=document.getElementById('viewerStage'),C=document.getElementById('viewerCanvas'),I=document.getElementById('viewerImg');
...
function zoomAt(next,ax,ay){{if(!I.naturalWidth||!I.naturalHeight)return;const sr=S.getBoundingClientRect(),ir=I.getBoundingClientRect(),rx=ir.width?(ax-ir.left)/ir.width:.5,ry=ir.height?(ay-ir.top)/ir.height:.5;zoomLevel=Math.max(.25,Math.min(5,next));I.style.width=Math.max(1,Math.round(I.naturalWidth*fitScale*zoomLevel))+'px';I.style.height=Math.max(1,Math.round(I.naturalHeight*fitScale*zoomLevel))+'px';requestAnimationFrame(()=>{{layoutCanvas();panState();S.scrollLeft=I.offsetLeft+rx*I.offsetWidth-(ax-sr.left);S.scrollTop=I.offsetTop+ry*I.offsetHeight-(ay-sr.top)}})}}
...
S.addEventListener('wheel',e=>{{if(!V.classList.contains('open')||!I.naturalWidth||e.target.closest('button,a'))return;e.preventDefault();const sr=S.getBoundingClientRect(),ir=I.getBoundingClientRect(),over=e.clientX>=ir.left&&e.clientX<=ir.right&&e.clientY>=ir.top&&e.clientY<=ir.bottom,ax=over?e.clientX:sr.left+S.clientWidth/2,ay=over?e.clientY:sr.top+S.clientHeight/2;zoomAt(zoomLevel*(e.deltaY<0?1.15:1/1.15),ax,ay)}},{{passive:false}});
S.addEventListener('pointerdown',e=>{{if(!V.classList.contains('open')||e.target.closest('button,a')||(e.pointerType==='mouse'&&e.button!==0))return;activePointers.set(e.pointerId,{{x:e.clientX,y:e.clientY}});S.setPointerCapture(e.pointerId);if(activePointers.size===1){{dragging=S.classList.contains('pannable');dragX=e.clientX;dragY=e.clientY;startLeft=S.scrollLeft;startTop=S.scrollTop;S.classList.toggle('dragging',dragging)}}else if(activePointers.size===2){{const [a,b]=[...activePointers.values()];pinchStartDistance=Math.hypot(b.x-a.x,b.y-a.y)||1;pinchStartZoom=zoomLevel;dragging=false;S.classList.remove('dragging')}}e.preventDefault()}});
S.addEventListener('pointermove',e=>{{if(!activePointers.has(e.pointerId))return;activePointers.set(e.pointerId,{{x:e.clientX,y:e.clientY}});if(activePointers.size>=2){{const [a,b]=[...activePointers.values()],distance=Math.hypot(b.x-a.x,b.y-a.y),midX=(a.x+b.x)/2,midY=(a.y+b.y)/2;zoomAt(pinchStartZoom*distance/pinchStartDistance,midX,midY);e.preventDefault();return}}if(dragging){{S.scrollLeft=startLeft-(e.clientX-dragX);S.scrollTop=startTop-(e.clientY-dragY);e.preventDefault()}}}});
function stopPointer(e){{activePointers.delete(e.pointerId);if(S.hasPointerCapture(e.pointerId))S.releasePointerCapture(e.pointerId);dragging=false;S.classList.remove('dragging');if(activePointers.size===1&&S.classList.contains('pannable')){{const p=[...activePointers.values()][0];dragging=true;dragX=p.x;dragY=p.y;startLeft=S.scrollLeft;startTop=S.scrollTop;S.classList.add('dragging')}}}}
S.addEventListener('pointerup',stopPointer);S.addEventListener('pointercancel',stopPointer);S.addEventListener('gesturestart',e=>e.preventDefault(),{{passive:false}});S.addEventListener('gesturechange',e=>e.preventDefault(),{{passive:false}});
</script></body></html>'''
...
for slug,name in people.items():
    display_name={'samatha':'Samatha Bell','breana':'BreeAna Lear','teri-wallace':'Teri Wallace Harrison','teresa-cooper':'Teresa Cooper Justice','carla-pittman':'Carla Pittman Lilly','peggy-faulkenberry':'Peggy Elliot Faulkenberry','lisa-steen':'Lisa Faulkenberry Steen','macey-steen':'Macey Steen Payne','rachel-bell':'Rachel Tharp Bell','alice-neely':'Alice Banks Neely','cindy-neely':'Cindy Neely Greene','kathleen-brown':'Kathleen Hinson Brown','mary-ellen-whitaker':'Mary Ellen Brown Whitaker'}.get(slug,name)
...
for slug,(name,description) in hobbies.items():
    ...
    after=(f'<section class="archive-support-section" aria-labelledby="ptsd-support-title" style="margin:32px 0"><h2 id="ptsd-support-title" style="border-bottom:1px solid #ccc;padding-bottom:6px">PTSD Support for Soldiers &amp; First Responders</h2>'
           f'<a href="tree_thumbs/ptsd_support_soldiers_first_responders.png" title="Open the PTSD support information at full size" style="display:block;max-width:1000px;margin:0 auto">'
           f'<img src="tree_thumbs/ptsd_support_soldiers_first_responders.png" alt="PTSD support for soldiers and first responders. Call or text 988. Veterans: dial 988, then press 1. Free, confidential, and available 24 hours a day." loading="lazy" style="display:block;width:100%;height:auto;border-radius:12px;box-shadow:0 2px 12px #0003"></a></section>') if slug=='military-rescue-police-ems' else ''
    section_name='Service & Public Safety' if slug=='military-rescue-police-ems' else 'Hobbies'
    page(f'{name} - {section_name}',subtitle,items,f'hobby-{slug}.html',empty,after)
