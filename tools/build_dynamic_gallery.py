...
def media_paths(x):
    ...
    if x.get('media_type')=='video' or p.startswith('videos/'):
        return (p,p,p)
...
def page(title,subtitle,items,filename):
    ...
    for y in sorted(years,key=lambda z:(z=='Unknown',z)):
        ...
        for x in years[y]:
            ...
            viewer.append({'view':v,'orig':o,'title':ttl,'date':date,'location':loc,'people':ppl,'people_raw':x.get('people') or [],'categories':x.get('categories') or [],'tags':x.get('tags') or [],'description':desc,'name':fn,'media_type':'video' if is_video else 'photo'})
            idx+=1
...
function mediaType(a){{if(a.media_type==='video')return 'Videos';const tags=vals(a.tags).map(norm);if(tags.includes('newspaper'))return 'Newspapers';if(tags.includes('document'))return 'Documents';if(tags.includes('sound'))return 'Sound';return 'Photos'}}
...
print('Generated gallery and',len(people),'person galleries from',len(DATA),'metadata records including video support')
