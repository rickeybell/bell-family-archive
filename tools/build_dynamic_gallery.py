...
def date_sort_key(x):
    ...
    if re.fullmatch(r'\d{4}',v):
        return (0, f'{v}-01-01', str(x.get('path') or '').lower())
    if re.fullmatch(r'\d{4}-\d{2}-\d{2}',v):
        return (0, v, str(x.get('path') or '').lower())
    folder=str(x.get('folder') or '')
    if re.fullmatch(r'\d{4}',folder):
        return (1, f'{folder}-12-31', str(x.get('path') or '').lower())
...
def clean_location_label(value):
    ...
    replacements={
        'Mason St Orig Bell Family Home':'Mason St',
        'Mason St Original Bell Family Home':'Mason St',
        'Mason Street Orig Bell Family Home':'Mason St',
        'Mason Street Original Bell Family Home':'Mason St',
    }
    return replacements.get(v,v)

def display_location(x):
    parts=[]
    loc=clean_location_label(x.get('location'))
...
def cards(items):
    ...
    for x in sorted(items,key=date_sort_key):
        ...
        if location:
            meta_lines.append(f'<div class="archive-meta-line">{esc(location)}</div>')
        if people:
            meta_lines.append(f'<div class="archive-meta-line">{esc(people)}</div>')
